    ...
    public async Task<IEnumerable<MemberDto>> GetUserLikes(string predicate, int userId)
    {
        ...
        switch (predicate)
        {
            ...
            default:
                ...
                return await likes.Where(x => x.TargetUserId == userId && likesIds.Contains(x.SourceUserId)).Select(x => x.SourceUser).ProjectTo<MemberDto>(mapper.ConfigurationProvider).ToListAsync();
        }
    }
